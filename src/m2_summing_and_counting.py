...
def run_test_sum_more_cosines():
    ...
    # Test 2:
    expected = -.139975  # This is APPROXIMATELY the correct answer.
    answer = sum_more_cosines(8, 13)
    ...
    # Test 3:
    expected = -.39980  # This is APPROXIMATELY the correct answer.
...
def count_sines_vs_cosines(m):
    ...
    count=0 #at 0 sin(0)<cos(0), therefore the integer 0 does not add to the count
    for k in range(m):
        if math.sin(k+1) > math.cos(k+1):
            count=count+1
        if math.sin(-(k+1)) > math.cos(-(k+1)):
            count=count+1
        else:
